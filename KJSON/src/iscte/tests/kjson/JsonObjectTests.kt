...
class JsonObjectTests {
    ...
    fun getCadeiraMutableJson() : MutableJsonObject {
        return MutableJsonObject(mutableMapOf(
            "unidade curricular" to JsonString("PA"),
            "nota" to JsonNumber(20),
            "aprovado" to JsonBoolean(true),
            "data de entrega" to JsonNull,
    ...
    fun testToJsonString() {
        ...
        val cadeiraJsonString = "{\"unidade curricular\": \"PA\", \"nota\": 20, \"aprovado\": true, \"data de entrega\": null, \"professor\": $professorJsonString}"
    ...
    fun testMutableOperations() {
        ...
        assertEquals(cadeiraJsonString, cadeiraJson.toJsonString())
        assertEquals(2025, ano.value)
        assertEquals(2025, anoRemoved.value)
    }
    ...
    fun testAcceptValue() {
        val cadeiraJson : MutableJsonObject = getCadeiraMutableJson()
        var jsonPairs = ""
        cadeiraJson.acceptValue { value ->
            jsonPairs += "${value.value} "
        }
        val jsonPairsString = "PA 20 true null André null true D6.23 "
